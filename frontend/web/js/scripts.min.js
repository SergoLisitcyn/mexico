...
$(function() {
  ...
  function getNumEnding(iNumber, aEndings)
  {
	...
	function fun1() {
		var rs_sum=document.getElementById('rs_sum'); //rng - это ползунок
		var rs_term=document.getElementById('rs_term'); //rng - это ползунок
		var i1=document.getElementById('cost'); // i1 - input
		var procent=document.getElementById('procent'); // i1 - input
		var first_loan=document.getElementById('first_loan'); // i1 - input

		var vat = 0.16;
		var rsSum = rs_sum.value;
		var sum = rsSum * (procent.value/100) * rs_term.value;
		var sumWithVat = sum * vat;
		var totalSum = Math.round(sum, 2) + Math.round(sumWithVat, 2);
		var allSum = Math.round(rsSum,2)+Math.round(totalSum, 2);
		if(first_loan.value == 1){
			allSum = rsSum;
		}

		i1.textContent = allSum;
	}

	function funmfo() {
		var rs_sum=document.getElementById('rs_sum'); //rng - это ползунок
		var rs_term=document.getElementById('rs_term'); //rng - это ползунок
		var added_item_button = document.getElementById('error_calc');
		var actualDisplay = getComputedStyle(added_item_button).display;
		jQuery.ajax({
			method: "POST",
			url: "count-mfo",
			data: {'rs_sum': rs_sum.value, 'rs_term' : rs_term.value},
		}).done(function (data) {
			if(data == 'SUCCESS') {
				$('#calculator__button_main_page').prop('disabled', false);
				$('#calculator__button_main_page').removeClass('calculator__disabled');
				added_item_button.style.display = 'none';
			} else {
				$('#calculator__button_main_page').prop('disabled', true);
				$('#calculator__button_main_page').addClass('calculator__disabled');
...
$(function() {
	...
	$(`.tabs-content__item[data-tab-content="${dataTab}"]`).css({'overflow': 'visible', 'height': '100%'});;
	...
	$('.tabs-control__item').on('click', (e) => {
			...
			$('.tabs-content__item').css({'overflow': 'hidden', 'height': '0'});;
			// $(`.tabs-content__item[data-tab-content="${dataTab}"]`).show();
			$(`.tabs-content__item[data-tab-content="${dataTab}"]`).css({'overflow': 'visible', 'height': '100%'});;
	});
	...
	$('.reviews-modal__close, .reviews-modal__overlay').click(function () {
		$('.reviews-modal')
			.animate({opacity: 0, top: '45%'}, 200, 
			function() { 
				$(this).css('display', 'none'); 
				$('.reviews-modal__overlay').fadeOut(400);
			}
		);
	});
